...
class AngularClientRenderer:
    ...
    def __init__(self, target_origin):
        # -- source repo
        self.template_repo = TemplateRepo()

        # -- target repo
        self.repo = AngularRepo(target_origin)
    ...
    def render(
        ...
        self.template_repo.stash()
        self.template_repo.pull()

        # -- copy its content to the temp directory into which client will get
        # -- render
        self.repo.clone()

        # self.repo --> clone it temp dir!
        # FIXME: here I must specify what is there to be published!!!
        self.template_repo.copy_to(self.repo.base_path, client_prefix)
    ...
    def render_api_index_ts(self, commands_by_domain):
        ...
        path = os.path.join(
            self.repo.base_path,
            'projects/cosphere-client/src/domains/index.ts')
        with open(path, 'w') as f:
            f.write('\n'.join(blocks))
    ...
    def render_api_ts(self, commands_by_domain):
        ...
        path = os.path.join(
            self.repo.base_path,
            'projects/cosphere-client/src/services/api.service.ts')
        with open(path, 'w') as f:
            f.write('\n\n'.join(blocks))
    ...
    def render_client_module_ts(self, commands_by_domain):
        ...
        path = os.path.join(
            self.repo.base_path,
            'projects/cosphere-client/src/cosphere-client.module.ts')
        with open(path, 'w') as f:
            f.write('\n\n'.join(blocks))
