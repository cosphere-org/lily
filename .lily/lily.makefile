...
migrations_apply_for_version:  ## apply migrations plan for specific version
	source env.sh && \
	python lily/manage.py apply_migration_plan_for_version $(version)
...
start_gunicorn: migrations_apply_current  ## start service locally
	source env.sh && \
	export PYTHONPATH="${PYTHONPATH}:${PWD}/lily" && \
	python lily/manage.py migrate && \
	gunicorn conf.wsgi \
		--worker-class gevent \
		-w 1 \
...
start_dev_server: migrations_apply_current  ## start development server (for quick checks) locally
	source env.sh && \
	python lily/manage.py runserver 127.0.0.1:${LILY_SERVICE_PORT}
